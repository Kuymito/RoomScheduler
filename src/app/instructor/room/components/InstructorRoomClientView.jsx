...
export default function InstructorRoomClientView({ initialAllRoomsData, buildingLayout, initialInstructorSchedules }) {
    const { data: session } = useSession();
    const token = session?.accessToken;

    // SWR hooks for data fetching and revalidation
    ...
    const [selectedTimeSlot, setSelectedTimeSlot] = useState(TIME_SLOTS[2]); // Default to Afternoon
    ...
    // Memoized derivation of class list for the request form
    const instructorClasses = useMemo(() => {
        return (initialInstructorSchedules || []).map(cls => ({
            id: cls.scheduleId,
            name: cls.className,
            shift: `${cls.shift.startTime}-${cls.shift.endTime}`,
        }));
    }, [initialInstructorSchedules]);

    // Memoized processing of the schedule to handle temporary assignments
    const processedScheduleMap = useMemo(() => {
        if (!scheduleMap) return {};

        const newScheduleMap = JSON.parse(JSON.stringify(scheduleMap));
        ...
        (initialInstructorSchedules || []).forEach(schedule => {
            (schedule.dayDetails || []).forEach(detail => {
                // Check for a valid temporary assignment
                if (detail.effectiveDate && detail.temporaryDay && detail.temporaryRoomId && detail.temporaryShift?.name) {
                    const effectiveDate = new Date(`${detail.effectiveDate}T00:00:00`);

                    // Process only if the change is within the current week
                    if (effectiveDate >= startOfWeek && effectiveDate <= endOfWeek) {
                        
                        // 1. Block the TEMPORARY room on the TEMPORARY day/shift
                        const tempDayName = effectiveDate.toLocaleDateString('en-US', { weekday: 'long' });
                        const tempShiftName = detail.temporaryShift.name;
                        if (!newScheduleMap[tempDayName]) newScheduleMap[tempDayName] = {};
                        if (!newScheduleMap[tempDayName][tempShiftName]) newScheduleMap[tempDayName][tempShiftName] = {};
                        newScheduleMap[tempDayName][tempShiftName][detail.temporaryRoomId] = `Temp: ${schedule.className}`;

                        // 2. Block the ORIGINAL room on the ORIGINAL day/shift
                        const originalDayName = detail.dayOfWeek.charAt(0) + detail.dayOfWeek.slice(1).toLowerCase();
                        const originalShiftName = schedule.shift.name;
                        if (!newScheduleMap[originalDayName]) newScheduleMap[originalDayName] = {};
                        if (!newScheduleMap[originalDayName][originalShiftName]) newScheduleMap[originalDayName][originalShiftName] = {};
                        if (schedule.roomId) {
                           newScheduleMap[originalDayName][originalShiftName][schedule.roomId] = `Moved: ${schedule.className}`;
                        }
                    }
                }
            });
    ...
    }, [scheduleMap, initialInstructorSchedules]);

    // Effect to update rooms data when SWR refetches
    useEffect(() => {
    ...
    const handleRoomClick = (roomId) => {
        ...
        setRoomDetails(allRoomsData[roomId] || null);
    };
    ...
    const handleSaveRequest = async (requestData) => {
        ...
        try {
            ...
            await notificationService.submitChangeRequest(payload, token);
    ...
    const isRequestDisabled = roomsLoading || schedulesLoading || !roomDetails || roomDetails.status === 'unavailable' || !!processedScheduleMap?.[selectedDay]?.[selectedTimeSlot]?.[roomDetails.id];
    ...
    return (
        <>
            ...
            {isFormOpen && (
                <Suspense fallback={<div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60"><div className="w-10 h-10 border-4 border-t-transparent border-white rounded-full animate-spin"></div></div>}>
                    <RequestChangeForm
                        isOpen={isFormOpen}
                                ...
                                <div key={floor} className="space-y-3">
                                    ...
                                    <div className={`grid gap-3 sm:gap-4 ${getGridColumnClasses(selectedBuilding, floor)}`}>
                                        {rooms.map((roomId) => {
                                            ...
                                            const scheduledClass = processedScheduleMap?.[selectedDay]?.[selectedTimeSlot]?.[room.id];
