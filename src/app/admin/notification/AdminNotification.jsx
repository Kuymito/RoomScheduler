...
const NotificationItem = ({ notification, onApprove, onDeny, onMarkAsRead }) => {
    // The notification object now includes the 'status' field
    const { id, avatarUrl, message, timestamp, isUnread, type, status, details } = notification;
        ...
        >
            <div className="flex flex-row items-start p-4 md:p-5 gap-4 relative animate-fade-in-scale">
                ...
                <div className="flex flex-col gap-2 flex-1 min-w-0">
                    ...
                    {/* --- THIS IS THE FIX --- */}
                    {/* The buttons are now only rendered if the status is "PENDING". */}
                    {status === 'PENDING' && (
