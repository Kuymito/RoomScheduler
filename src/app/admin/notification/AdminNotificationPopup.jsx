...
const NotificationPopup = ({ show, notifications = [], onMarkAllRead, onApprove, onDeny, onMarkAsRead, anchorRef, onClose }) => {
    const { data: session } = useSession(); // Use session to get the token
    ...
    // --- FIX: Implement the handler for "Mark all as read" ---
    const handleMarkAllRead = async () => {
        // Check for session and if there are any notifications to mark
        if (session?.accessToken && notifications.length > 0) {
            try {
                // Call the service function that iterates and marks all as read
                await notificationService.markAllNotificationsAsRead(notifications, session.accessToken);

                // Call the parent's handler, which should trigger a UI update or refetch
        ...
        >
            <div className="relative flex justify-between items-center py-4 px-5">
                ...
                <div className="flex items-center gap-3">
                    {/* The button now calls the implemented handler */}
                    <button
