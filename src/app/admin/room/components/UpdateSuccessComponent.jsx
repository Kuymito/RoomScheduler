...
// Optimized Checkmark Icon (remains the same)
const CheckmarkIcon = (props) => (
  <svg
    {...props}
    xmlns="http://www.w3.org/2000/svg"
    ...
    strokeWidth={1.5}
    stroke="currentColor"
    className="text-green-500" // Keep or make dynamic if needed
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
...
// Optimized Close Icon (remains the same)
const CloseIconSvg = (props) => (
  <svg
    {...props}
    xmlns="http://www.w3.org/2000/svg"
...
const SuccessAlert = ({
  onConfirm,
  onClose,
  title: propTitle, // Rename to avoid conflict with t function result
  messageLine1: propMessageLine1,
  messageLine2: propMessageLine2,
  ...
  const title = propTitle || t('successAlert.title');
  const messageLine1 = propMessageLine1 || t('successAlert.messageLine1');
  const messageLine2 = propMessageLine2 || t('successAlert.messageLine2');
  const confirmButtonText = propConfirmButtonText || t('successAlert.confirmButtonText');
  ...
  return (
    ...
    >
      {onClose && (
        <button
          onClick={handleClose}
          aria-label={t('successAlert.closeAlertLabel')} // Translated aria-label
          className="absolute w-[20px] h-[20px] left-[431px] top-[21px] text-alert-close-icon hover:opacity-75 transition-opacity"
        >
          <CloseIconSvg className="w-full h-full" />
        </button>
