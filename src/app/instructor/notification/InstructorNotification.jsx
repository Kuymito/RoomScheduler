...
const StatusIcon = ({ status }) => {
    switch (status) {
        ...
        default:
             // For general info or other statuses, show a generic info icon
            return <InfoCircleIcon className="w-6 h-6 text-blue-500 bg-white rounded-full absolute -bottom-1 -right-1" />;
    }
};
...
const InstructorNotification = ({ notification, onMarkAsRead }) => {
    ...
    // Destructure properties from the notification object.
    const { notificationId, message, createdAt, read, type, details } = notification;
    const isUnread = !read;

    // Determine the status from the message content for icon display
    const getStatusFromMessage = () => {
        const lowerMessage = message.toLowerCase();
        if (lowerMessage.includes('approved')) return 'APPROVED';
        if (lowerMessage.includes('denied') || lowerMessage.includes('rejected')) return 'DENIED';
        return 'INFO'; // Default status for general notifications
    };
    const status = getStatusFromMessage();
    ...
    const AvatarPlaceholder = ({ name }) => (
        <div className="w-12 h-12 rounded-full bg-blue-600 text-white flex items-center justify-center text-lg font-semibold flex-shrink-0">
            {name ? name.substring(0, 2).toUpperCase() : 'NU'}
        </div>
    );
        ...
        >
            <div className="flex flex-row items-start p-4 md:p-5 gap-4 relative animate-fade-in-scale">
                ...
                <div className="relative flex-shrink-0">
                    {/* Use a placeholder since instructor profile pics aren't in this notification object */}
                    <Image
