...
const NotificationItem = ({ notification, onApprove, onDeny }) => {
  ...
  return (
    <div className={`w-full ${isUnread ? 'bg-blue-50' : 'bg-white'}`}> {/* Lighter blue for unread, or use your #EDF3FF */}
      <div className="flex flex-row items-start p-4 md:p-5 gap-4 relative"> {/* Standardized padding and slightly increased gap */}
        ...
        {/* Avatar */}
        <div 
          className="w-12 h-12 rounded-full flex-shrink-0 bg-cover bg-center bg-slate-200" // Fallback bg color
        ...
        <div className="flex flex-col gap-2 flex-1 min-w-0">
          <p className="font-inter font-semibold text-xs sm:text-sm leading-normal text-slate-700 self-stretch break-words">
          ...
          {type === 'roomRequest' && (
            <div className="flex flex-row items-center pt-1.5 gap-2.5"> {/* Slightly more spacing */}
              ...
              <button
                onClick={() => onDeny(id)}
                className="box-border flex justify-center items-center py-1.5 px-4 bg-white border border-slate-300 rounded-md text-slate-700 font-inter font-medium text-xs hover:bg-slate-50 active:bg-slate-100 transition-colors shadow-sm"
        ...
        <div className="flex flex-col items-end text-right flex-shrink-0 pl-2 ml-auto w-auto min-w-[50px]"> {/* Ensure some min-width for timestamp */}
          <span className="font-inter font-normal text-xs leading-normal text-slate-500 whitespace-nowrap">
            {timestamp}
          </span>
      ...
      <div className="flex flex-row">
        ...
        <div className="pl-[84px] w-full"> {/* Using the 84px from your original spec calculation */}
            <div className="h-px border-t border-slate-200 w-full"></div> {/* Lighter divider */}
        </div>
      </div>
    </div>
  );
