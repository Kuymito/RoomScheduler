...
import RoomPageSkeleton from "@/app/admin/room/components/RoomPageSkeleton"; // Can reuse admin skeleton
...
const RoomViewContent = () => {
  ...
  const buildings = {
    "Building A": [{ floor: 5, rooms: ["A1", "A2", "A3", "A4", "A5"] },{ floor: 4, rooms: ["B1", "B2"] },{ floor: 3, rooms: ["C1", "C2"] },{ floor: 2, rooms: ["D1", "D2"] },{ floor: 1, rooms: ["E1", "E2"] },],
    ...
    <>
      {showSuccessAlert && ( 
        <SuccessAlert
          ...
          title="Request Sent"
          messageLine1={`Your request for room ${roomDetails?.name || ""} was sent successfully.`}
          messageLine2="You will be notified upon approval."
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className={`flex-1 rounded-b-md p-2 flex flex-col justify-center items-center ${isAvailable ? 'bg-white dark:bg-slate-800' : 'bg-slate-50 dark:bg-slate-800/50'}`}>
                                                <span className={`font-semibold text-xs ${isAvailable ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>{room?.status}</span>
