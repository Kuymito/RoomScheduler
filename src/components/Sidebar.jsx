...
const Sidebar = ({ isCollapsed, activeItem, onNavItemClick, navigatingTo }) => {
    ...
    return (
        <div id="sidebar" className={`sidebar fixed top-0 h-screen bg-white dark:bg-gray-900 shadow-custom-medium py-5 flex flex-col transition-all duration-300 ease-in-out z-40`} style={{ width: isCollapsed ? '80px' : '265px' }}>
            <div className={`logo h-[50px] mb-5 flex items-center justify-center ${isCollapsed ? 'px-0' : 'px-5'}`}>
                ...
                <span className={`logo-text-collapsed font-bold text-lg text-black ${isCollapsed ? 'block' : 'hidden'}`}>
                    <Image src="/images/LOGO-NUM-1.png" alt="NUM Logo" width={32} height={32} className="inline-block" />
                </span>
            </div>
            ...
            <div className="profile-info flex flex-col items-center my-7 overflow-hidden">
                <div className={`profile-avatar rounded-full mb-2.5 flex justify-center items-center ${isCollapsed ? 'w-10 h-10' : 'w-20 h-20'}`}>
                    {isLoading ? (
                        <div className={`rounded-full bg-gray-300 dark:bg-gray-600 animate-pulse ${isCollapsed ? 'w-10 h-10' : 'w-20 h-20'}`}></div>
                    ) : (
                        <AdminAvatarIcon className={`text-gray-700 dark:text-gray-400 ${isCollapsed ? 'w-10 h-10' : 'w-20 h-20'}`} />
                    )}
                </div>
