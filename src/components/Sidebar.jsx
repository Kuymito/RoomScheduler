...
//     // Use the translation function 't' for labels
//     const navItemsData = [
//         { id: 'dashboard', href: '/admin/dashboard', icon: DashboardIcon, label: t('sidebar.dashboard') },
//         { id: 'class', href: '/admin/class', icon: ClassIcon, label: t('sidebar.class') },
//         { id: 'instructor', href: '/admin/instructor', icon: InstructorIcon, label: t('sidebar.instructor') },
//         { id: 'room', href: '/admin/room', icon: RoomIcon, label: t('sidebar.room') },
//         { id: 'schedule', href: '/admin/schedule', icon: ScheduleIcon, label: t('sidebar.schedule') },
//     ];
...
const NavItem = ({ href, icon: Icon, label, isActive, isCollapsed, onClick, isNavigating, currentLanguageCode }) => (
    <Link
        href={href}
        onClick={onClick}
        className={`nav-item flex items-center py-2.5 mb-1.5 rounded-[5px] cursor-pointer overflow-hidden transition-all duration-150 ease-in-out hover:bg-gray-100 dark:hover:bg-gray-800 ${isActive ? 'hover:bg-sky-100 dark:hover:bg-blue-800' : ''} ${isCollapsed ? 'px-0 justify-center' : 'px-[60px]'} ${isNavigating ? 'opacity-70' : ''}`}
    >
      ...
      <div className={`nav-text text-xs whitespace-nowrap transition-all duration-150 ease-in-out ${isActive ? 'text-num-blue' : 'text-[#737373] dark:text-gray-300'} ${isCollapsed ? 'opacity-0 max-w-0 hidden' : 'opacity-100 max-w-[150px]'} ${currentLanguageCode === 'km' ? 'font-battambang' : ''}`}>
        {label}
      </div>
    </Link>
);
...
const Sidebar = ({ isCollapsed, activeItem, onNavItemClick, navigatingTo, profile, isProfileLoading, currentLanguageCode }) => {
    const { t } = useTranslation(); // Get the translation function
    const user = profile;
    const isLoading = isProfileLoading;
    ...
    return (
        <div id="sidebar" className={`sidebar fixed h-full bg-white dark:bg-gray-900 shadow-custom-medium py-5 flex flex-col transition-all duration-300 ease-in-out z-40`} style={{ width: isCollapsed ? '80px' : '265px' }}>
            <div className={`logo h-[50px] mb-5 flex items-center justify-center ${isCollapsed ? 'px-0' : 'px-5'}`}>
                <Image src="/images/LOGO-NUM-1.png" alt="NUM Logo" width={isCollapsed ? 0 : 150} height={50} className={`logo-img ${isCollapsed ? 'hidden' : 'block h-[50px]'}`} style={{ width: isCollapsed ? 0 : 'auto' }} />
                <span className={`logo-text-collapsed font-bold text-lg text-black ${isCollapsed ? 'block' : 'hidden'} ${currentLanguageCode === 'km' ? 'font-battambang' : ''}`}>
                    <Image src="/images/LOGO-NUM-1.png" alt="NUM Logo" width={32} height={32} />
                </span>
            </div>
            <hr className="border-t border-num-gray-light dark:border-gray-700" />
            ...
            <nav className="nav-menu flex-grow mt-5 px-2">
                {navItemsData.map((item) => (
                    <NavItem
                        ...
                        currentLanguageCode={currentLanguageCode} // Pass the language code here
                    />
                ))}
            </nav>
        </div>
