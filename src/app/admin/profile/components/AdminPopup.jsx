...
const AdminPopup = ({ show, onLogoutClick, adminName, adminEmail, isNavigating, onNavigate }) => {
  ...
  return (
    <div className="admin-popup absolute sm:w-[299px] w-[220px] right-5 top-20 bg-white dark:bg-gray-700 shadow-custom-heavy rounded-[5px] z-[1000]">
      <div className="admin-popup-header w-full h-[84px] bg-num-blue dark:bg-blue-800 rounded-t-[5px] flex flex-col items-center justify-center p-2.5 overflow-hidden">
        {/* UPDATED: Added truncate and title attribute for better UX on long names */}
        <div 
          className="max-w-[200px] admin-popup-name font-poppins font-medium text-base leading-6 text-white truncate"
          title={adminName}
        >
          {adminName}
        </div>
        {/* UPDATED: Added truncate and title attribute for better UX on long emails */}
        <div 
          className="max-w-[150px] admin-popup-email font-sans font-normal text-xs leading-4 text-num-blue-light truncate"
          title={adminEmail}
        >
          {adminEmail}
        </div>
      ...
      <div className="admin-popup-options p-2.5 flex flex-col">
        ...
        >
          {isNavigating ? (
            ...
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="sm:size-4 size-2 text-num-blue dark:text-blue-400">
