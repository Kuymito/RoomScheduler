...
const RoomCard = ({ cellData, isDragOver, isWarning, dragHandlers }) => {
    ...
    return (
        <div className={`rounded-lg border-2 flex flex-col transition-all duration-150 overflow-hidden ${getBorderColor()}`}>
            <div onClick={handleHeaderClick} className={`px-2 py-1 flex justify-between items-center border-b-2 transition-colors cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 ${isWarning ? 'bg-red-100 dark:bg-red-800/50' : 'bg-gray-50 dark:bg-gray-800'}`}>
...
export default function ScheduleClientView({ initialClasses, initialSchedules, constants }) {
    ...
    // --- Drag Handlers (identical to original logic) ---
        ...
        <>
            ...
            <div className='p-6 dark:text-white flex flex-col lg:flex-row gap-6 h-[calc(100vh-100px)]'>
                <div className='w-full lg:w-[260px] xl:w-[300px] flex-shrink-0 p-4 bg-white dark:bg-gray-900 border dark:border-gray-700 shadow-lg rounded-xl flex flex-col'>
                    ...
                    <div className="flex items-center flex-row gap-2 mb-2 "><div className="w-1/2"><select id="degree-select" value={selectedDegree} onChange={(e) => setSelectedDegree(e.target.value)} className="w-full mt-1 p-2 text-sm border border-gray-300 rounded-md dark:bg-gray-800 dark:border-gray-600 dark:text-gray-200 focus:ring-sky-500 focus:border-sky-500"><option value="All">Degrees</option>{degrees.map(d => <option key={d} value={d}>{d}</option>)}</select></div><div className="w-1/2"><select id="generation-select" value={selectedGeneration} onChange={(e) => setSelectedGeneration(e.target.value)} className="w-full mt-1 p-2 text-sm border border-gray-300 rounded-md dark:bg-gray-800 dark:border-gray-600 dark:text-gray-200 focus:ring-sky-500 focus:border-sky-500"><option value="All">Generations</option>{generations.map(g => <option key={g} value={g}>{g}</option>)}</select></div></div>
                ...
                <div className='flex-1 p-4 sm:p-6 bg-white dark:bg-gray-900 border dark:border-gray-700 shadow-xl rounded-xl flex flex-col overflow-y-auto'>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center gap-2"><div className="flex items-center gap-2"><div className="flex items-center"><label htmlFor="building-select" className="text-sm font-medium text-slate-600 dark:text-slate-400">Building:</label><select id="building-select" value={selectedBuilding} onChange={(e) => setSelectedBuilding(e.target.value)} className="p-2 text-sm rounded-md dark:bg-gray-800 dark:text-gray-200">{buildings.map(b => <option key={b} value={b}>{b}</option>)}</select></div></div><hr className="flex-1 border-t border-slate-300 dark:border-slate-700" /></div>
                    <div className="flex-grow flex flex-col gap-y-4 mt-4">
                        {currentGrid.map((floor, floorIndex) => (
                            <div key={floorIndex}>
                                ...
                                <div className="grid gap-3" style={{ gridTemplateColumns: `repeat(${gridDimensions.cols}, 1fr)`}}>{floor.map((cellData, roomIndex) => (<RoomCard key={cellData.room.id} cellData={cellData} isDragOver={dragOverCell?.floorIndex === floorIndex && dragOverCell?.roomIndex === roomIndex} isWarning={warningCellId === cellData.room.id} dragHandlers={{ onDragOver: handleGridCellDragOver, onDragEnter: (e) => handleGridCellDragEnter(e, floorIndex, roomIndex), onDragLeave: handleGridCellDragLeave, onDrop: (e) => handleGridCellDrop(e, floorIndex, roomIndex), onDragStart: (e) => handleDragStartFromGrid(e, cellData.class, floorIndex, roomIndex), onDragEnd: handleDragEnd, }} />))}</div>
                            </div>
                        ))}
                    </div>
                </div>
