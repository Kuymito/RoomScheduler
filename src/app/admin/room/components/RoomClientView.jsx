...
export default function RoomClientView({ initialAllRoomsData }) {
    ...
    const buildings = {
        ...
        "Building F": [
            { floor: 3, rooms: ["F11", "F12", "F13", "F14", "F15", "F16", "F17", "F18", "F19", "F20"] },
                                    ...
                                    <div className="floor-section">
                                        ...
                                        <div className="grid xl:grid-cols-5 lg:grid-cols-2 md:grid-cols-2 gap-3 ">
                                            {rooms.map((roomId) => (
                                                <div
                                                    key={roomId}
                                                    className={`h-[90px] sm:h-[100px] border rounded-md flex flex-col cursor-pointer transition-all duration-150 shadow-sm hover:shadow-md bg-white dark:bg-slate-800 ${selectedRoom === roomId ? "border-blue-500 ring-2 ring-blue-500 dark:border-blue-500" : "border-slate-300 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-600"}`}
                                                ...
                                                >
                                                    <div className="h-[30px] rounded-t-md flex items-center justify-center px-2 relative border-b border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700">
                                                        {/* Simplified condition: green by default, blue only if selected */}
                                                        <div className={`absolute left-2 top-1/2 -translate-y-1/2 w-2 h-2 ${selectedRoom === roomId ? 'bg-blue-500' : 'bg-green-500'} rounded-full`}></div>
                                                        <span className={`ml-3 text-xs sm:text-sm font-medium ${selectedRoom === roomId ? 'text-blue-700 dark:text-blue-300' : 'text-slate-700 dark:text-slate-300'}`}>{allRoomsData[roomId]?.name || roomId}</span>
                                                    </div>
                                                    <div className="flex-1 rounded-b-md p-2 flex flex-col justify-center items-center bg-white dark:bg-slate-800">
                                                        <span className={`text-xs text-slate-500 dark:text-slate-400 ${selectedRoom === roomId ? 'text-slate-600 dark:text-slate-300' : ''}`}>Capacity: {allRoomsData[roomId]?.capacity}</span>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
